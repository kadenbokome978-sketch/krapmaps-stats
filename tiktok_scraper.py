        ...
        try:
            ...
            if created_iso:
                ...
                try:
                    ...
                    date_str = datetime.utcfromtimestamp(int(item["createTime"])).strftime("%Y-%m-%d")
                except:
                    date_str = ""
            else:
                date_str = ""
...
def merge_videos(existing, fresh):
    ...
    for v in existing:
        vid_id = str(v.get("id",""))
        if vid_id:
            existing_by_id[vid_id] = v
        # Also index by URL to catch same video with different ID formats
        url = v.get("url","")
        if url:
            existing_by_id[url] = v
    updated = 0
    added = 0
    now = datetime.utcnow()
    ...
    for fv in fresh:
        ...
        # Check by ID or URL to prevent doubles
        if vid_id in existing_by_id or (url and url in existing_by_id):
            if url and url in existing_by_id and vid_id not in existing_by_id:
                # Same video different ID — update the existing entry's ID
                existing_by_id[vid_id] = existing_by_id[url]
                existing_by_id[vid_id]["id"] = vid_id
